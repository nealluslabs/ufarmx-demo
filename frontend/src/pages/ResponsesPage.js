...
setTimeout(()=>{
 ...
 /* const forcedId =  []
  ...
  useEffect(()=>{
    ...
    const data = currentResponsesToDisplay && currentResponsesToDisplay
          ...
          <Grid item xs={12} md={4} lg={4}>
            <Paper
              sx={{
                ...
                backgroundColor:'inherit' /*'#F8F8F8'*/,
                borderRadius: '10px',
                
              }}
            >
              <DashboardCard header={'Total Responses'} value={currentResponsesToDisplay ?currentResponsesToDisplay.length :'0'} img={TeacherImg} additions={farmersAddedPerMonth ?farmersAddedPerMonth[farmersAddedPerMonth.length-1]:'0'} />
            </Paper>
          </Grid>
