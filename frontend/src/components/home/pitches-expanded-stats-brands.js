...
const columns = [
  ...
  },*/
  {
    ...
    renderCell: (params) => {
      ...
      return <div style={{display:"flex",alignItems:"center",justifyContent:"flex-start",gap:"1.5rem",width:"60%"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"flex-start"}}><div>{/*THE NUMBERS SHOULD GO HERE */}</div> <img src={params.row.image/*params.row.id === '0S91dTHhu7t0Zc6645Gb'?redboy:params.row.id === '75LPiOJKwtndeC67o5d3'?greenboy:params.row.id === '8Gnbs3WPwJ7ZzzvHgORs'?athlete:amfootball*/} style ={{height:"40px",width:"50px",borderRadius:"8px"}}/></div>
  ...
  { field: 'class', headerName: 'Executed', width: 300, renderCell: (params) => {
    return <div>{/*params.row.class*/new Date().toISOString}</div>;
  },  },
...
export default function PitchesExpandedStatsBrands({ brands }) {
