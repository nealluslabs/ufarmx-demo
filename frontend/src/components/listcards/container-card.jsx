...
const useStyles = makeStyles((theme) => ({
  row: {
    ...
    [theme.breakpoints.down('sm')]: {
      flexDirection: 'column', // Change to column for small screens and below
    },
  },
  text: {
   // width: '80%',
  ...
  collection: {
    fontWeight: 500,
    fontSize: '3rem', // Default for larger screens
    [theme.breakpoints.down('sm')]: {
...
const ContainerCard = ({data,index,user,pic,collection,about,signed,containerName}) => {
    ...
    <div className={classes.row}>
      ...
      <div className={classes.text}>
        <div style={{ color: 'black',display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center" }}>
          ...
          <img src={pic} style={{marginLeft:"2rem",marginBottom:"1rem",height:"10rem",width:"10rem",borderRadius:"1.5rem"}} alt="athlete image"/>
        </div>
      ...
      <div style={{ display:"flex",flexDirection:"column",justifyContent:{xs:'center',md:'flex-start'},gap:"1px" ,width:"75%",paddingLeft:{xs:"0rem",lg:"8rem"}}}>
          
          <div style={{fontWeight:"500",fontSize:"1.2rem",marginBottom:"-0.6rem"}}> {about} </div>
          <div className={classes.collection} > {collection} </div>
          <div  style={{fontWeight:"500",fontSize:"1.2rem",marginTop:"-0.6rem"}}> {signed}  </div>
